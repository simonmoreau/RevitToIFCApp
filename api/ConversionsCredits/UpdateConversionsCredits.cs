...
namespace api
{
  public class UpdateConversionsCredits
  {
    ...
    public UpdateConversionsCredits(GraphServiceClient graphServiceClient)
    {
        ...
        ILogger log)
    {
      try
      {
        ...
        if (session != null)
        {
          ...
          int newCreditsNumber = await UpdateCustomAttributeByUserId(updateCreditsBody.userId, log, credits);
    ...
    private async Task<int> UpdateCustomAttributeByUserId(string userId, ILogger log, int addedCreditsNumber)
    {
      ...
      log.LogInformation($"User with object ID '{userId}' successfully updated, with {existingCredits + addedCreditsNumber} credits.");
