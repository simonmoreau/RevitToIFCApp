    ...
    public partial class UploadedFiles
    {
        ...
        private async Task UploadFile(RevitFile revitFile)
        {
            ...
            if (eTags.Count == signedsUrlResponse.Urls.Count)
            {
                ...
                //5 Create a workItem
                WorkItemStatus createdWorkItemStatus = await _dataService.CreateWorkItem(objectKey, revitFile.Version, revitFile.Name);
                ...
                while (true)
                {
                    ...
                    switch (status.Status)
                    {
                        ...
                        case Status.FailedUploadOptional:
                            ...
                            return;
                        case Status.Success:
                            Signeds3downloadResponse signedDownload = await _dataService.GetDownloadUrl(objectKey, revitFile.Name);
